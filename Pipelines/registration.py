...
def launch(parent):
    window = RegistrationWindow(parent)
    window.show()
...
class RegistrationWindow(QMainWindow):
    ...
    def __init__(self, parent):
        ...
        # # Tab1
        # self.tab1 = MouseTab(self)
        
        # # Tab 2
        # self.tab2 = KeyboardTab(self)
        
        # self.tabs.addTab(self.tab1, "Mouse")
        # self.tabs.addTab(self.tab2, "Keyboard")
        
        self.registration = MouseTab(self)
...
class KeyboardTab(QWidget):
    ...
    def __init__(self, parent):
        ...
        self.ref_sequence = ""
        self.subjects = []
        self.ses_to_register = ""
        self.sess_to_register = []
        self.ref_ses = ""
        self.name_reg = ""
        self.trans_seq = ""
        
        self.select_session_to_register = QLineEdit(self)
        self.select_session_to_register.setPlaceholderText("Select session to register")
        
        self.select_sequence_to_register = QLineEdit(self)
        self.select_sequence_to_register.setPlaceholderText("Select sequence to register")
        
        self.select_ref_session = QLineEdit(self)
        self.select_ref_session.setPlaceholderText("Select reference session")
        
        self.select_ref_sequence = QLineEdit(self)
        self.select_ref_sequence.setPlaceholderText("Select reference sequence")
        ...
        self.same_transformation = QLabel("Apply same transformation ?")
        self.trans_sequence = QLineEdit(self)
        self.trans_sequence.setPlaceholderText("Sequence to apply same transformation")
        
        self.layout = QGridLayout()
        self.layout.addWidget(self.select_session_to_register, 0, 0, 1, 1)
        self.layout.addWidget(self.select_sequence_to_register, 0, 1, 1, 1)
        self.layout.addWidget(self.select_ref_session, 1, 0, 1, 1)
        self.layout.addWidget(self.select_ref_sequence, 1, 1, 1, 1)
        self.layout.addWidget(self.subjects_input, 2, 0, 1, 1)
        self.layout.addWidget(self.sessions_input, 2, 1, 1, 1)
        self.layout.addWidget(self.select_name_reg, 3, 0, 1, 2)
        self.layout.addWidget(self.same_transformation, 4, 0, 1, 2)
        self.layout.addWidget(self.trans_sequence, 5, 0, 1, 2)
        self.layout.addWidget(self.registration_button, 6, 0, 1, 2)
        
        self.setLayout(self.layout)
            
    def registration(self):
        logging.info('Registration')
        self.ses_to_register = self.select_session_to_register.text()
        self.sequence_to_register = self.select_sequence_to_register.text()
        self.ref_ses = self.select_ref_session.text()
        self.ref_sequence = self.select_ref_sequence.text()
        self.subjects = self.subjects_input.text().split(',')
        self.sess_to_register = self.sessions_input.text().split(',')
        self.name_reg = self.name_reg.text()
        self.trans_seq = self.trans_sequence.text()
        same_ses = self.ses_to_register == self.ref_ses
        if self.name_reg == "":
            reg = self.ref_sequence.split('_')[-1]
            self.name_reg = f'reg-{reg}'
            if not same_ses:
                self.name_reg = self.name_reg + f'-{self.ref_ses}'
